...
const Home = () => {
  ...
  const [activeIndex, setActiveIndex] = useState(2); // "Add Entry" index
  ...
  // 1. Initial positioning on mount
  useEffect(() => {
    if (scrollRef.current) {
      const container = scrollRef.current;
      const items = container.querySelectorAll('.nav-scroll-item');
      const target = items[2]; // Index 2 is "Add Entry"
      if (target) {
  ...
  return (
    <div className="relative min-h-screen overflow-x-hidden pb-44" style={{ backgroundColor: 'var(--bg-primary)' }}>
      ...
      <header className="p-6 pt-12 flex justify-between items-center">
        ...
        <button onClick={() => navigate('/reports')} className="p-4 rounded-2xl bg-[var(--bg-secondary)] border border-white/5 active:scale-95 transition-transform">
      ...
      <section className="px-6 mb-8">
        <motion.div 
          ...
          className="p-8 rounded-[3rem] bg-[var(--bg-secondary)] border border-white/5 shadow-2xl relative"
        >
          <p className="text-[10px] uppercase tracking-widest opacity-40 font-black mb-2" style={{ color: 'var(--text-main)' }}>Cycle Liquidity</p>
          ...
          <div className="grid grid-cols-2 gap-4">
             <div className="flex flex-col border-r border-white/10 pr-4">
               ...
               <div className="flex items-center gap-1 font-black text-sm" style={{ color: 'var(--text-main)' }}><ArrowUpRight size={14} className="text-emerald-500" /> {formatAmount(totals.income)}</div>
             </div>
             <div className="flex flex-col pl-4">
               <span className="text-[9px] uppercase font-black text-rose-500 mb-1">Outflow</span>
               <div className="flex items-center gap-1 font-black text-sm" style={{ color: 'var(--text-main)' }}><ArrowDownLeft size={14} className="text-rose-500" /> {formatAmount(totals.spend)}</div>
             </div>
          </div>
        </motion.div>
      </section>
      ...
      <section className="px-6 mb-10">
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {transactions.map((tx) => (
              <div key={tx._id} className="bg-[var(--bg-secondary)] p-5 flex justify-between items-center rounded-[2.5rem] border border-black/5 mx-2">
                <div className="flex items-center gap-4">
      ...
      <div className="fixed bottom-8 left-0 right-0 h-24 z-50 flex items-center justify-center pointer-events-none">
        ...
        {/* Static Selection Glass (The Swipe Target) */}
        <div 
          className="absolute w-16 h-16 rounded-[1.8rem] z-20 border border-white/10" 
          style={{ backgroundColor: 'var(--brand-color)', opacity: 0.9, boxShadow: `0 0 20px -5px var(--brand-color)` }}
        />
